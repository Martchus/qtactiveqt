...
/*
    Enumerations that originate from a COM type library are registered in the
    meta object under their unqualified name (e.g. "MyEnum"). Method and property
    signatures generated by dumpcpp, however, may refer to such a type fully
    qualified with the type library's namespace (e.g. "MyTypeLib::MyEnum").
    Look the enumerator up by its exact name first and, if that fails, retry with
    the unqualified tail so the qualified spelling still resolves. Returns the
    enumerator index, or -1 if none matches.
*/
static int enumeratorIndex(const QMetaObject *mo, const QByteArray &typeName)
{
    int index = mo->indexOfEnumerator(typeName);
    if (index == -1) {
        const qsizetype sep = typeName.lastIndexOf("::");
        if (sep != -1)
            index = mo->indexOfEnumerator(typeName.mid(sep + 2));
    }
    return index;
}
...
int QAxBase::internalInvoke(QMetaObject::Call call, int index, void **v)
{
    ...
    for (p = 0; p < int(params.cArgs); ++p) {
        ...
        QVariant qvar;
        // A parameter type recognized as a COM enum is always packed as int,
        // regardless of what QMetaType::fromName(type) resolves to: dumpcpp
        // qualifies enum parameter types with the type library's namespace,
        // and that qualified name can coincide with a real, but unrelated,
        // registered QMetaType (e.g. when the enum is itself declared at
        // that namespace scope), which must not be mistaken for the
        // parameter's actual type.
        if (enumeratorIndex(mo, type) != -1) {
    ...
    if (varc) {
        ...
        for (qsizetype i = 0; i < varc; ++i) {
            ...
            if ((!parse && d->useMetaObject && vType == QMetaType::QString)
                ...
                int enumIndex = enumeratorIndex(mo, paramType);
                if (enumIndex != -1) {
                    QMetaEnum metaEnum =mo->enumerator(enumIndex);
